...
REPO_URL = 'https://github.com/kyreniabupt/yyy.git'
...
def _update_database(source_folder):
    ...
    if result.failed:
        ...
        # 方法2：重置迁移（更彻底但会丢失数据）
        run(f'cd {source_folder} && rm -f db.sqlite3')
        run(f'cd {source_folder} && find . -path "*/migrations/*.py" -not -name "__init__.py" -delete')
        run(f'cd {source_folder} && find . -path "*/migrations/*.pyc" -delete')
